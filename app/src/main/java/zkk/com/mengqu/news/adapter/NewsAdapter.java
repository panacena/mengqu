package zkk.com.mengqu.news.adapter;
...
public class NewsAdapter extends BaseAdapter {
    ...
    private List<NewsItemBean> newsBeen=new ArrayList<NewsItemBean>();

    private Context context;
    public NewsAdapter(List<NewsItemBean> newsBeanList, Context con) {
        this.newsBeen = newsBeanList;
        this.context=con;
    }
    ...
    public int getItemViewType(int position) {
        if(newsBeen.get(position).getType()==0){
            return  TYPE_DATE;
        }else {
            return  TYPE_NOTDATE;
        }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        final NewsItemBean infoItemBean=newsBeen.get(position);
